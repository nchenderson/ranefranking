Qfn <- function(beta, tausq, ses, H=1) {
  ## ses - standard errors squared.
  resids <- as.numeric(Y - X%*%beta)
  B <- tausq/(ses + tausq)
  VV <- sqrt(B/(2*ses + tausq))

  if(H==1) {
    term1 <- sqrt(tausq/(2*pi))*mean(VV*dnorm(VV*resids))
  } else if(H==3) {
    ...
    f1 <- sqrt(tausq/(2*pi))
    term1 <- (5/8)*f1*mean(VV*der1) + (1/12)*f1*tausq*mean((VV^3)*der3) + (1/40)*f1*tausq*tausq*mean((VV^5)*der5)
  }
