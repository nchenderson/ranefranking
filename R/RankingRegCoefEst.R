RankingRegCoefEst <- function(y, X, ses, tau.sq, H=1, method="optim") {
  ...
  Qfn <- function(beta, H, VV) {
   ...
   if(ncol(X)==1 & method=="optim") {
       if(H==1) {
          ...
          beta.rank <- optimize(Qfn, lower=-10,upper=10, H=5, VV=VV)$minimum
       }
       ...
       if(H==1) {
          ...
          beta.rank <- optim(rep(0, ncol(X)),fn=Qfn, H=5, VV=VV)$par
       }
   } else if(method=="MM") {
